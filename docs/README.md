# Documentation

Guides for **fedora-linux-scripts**.

| Doc | Audience | Purpose |
|-----|----------|---------|
| **[GETTING-STARTED.md](GETTING-STARTED.md)** | Everyone | Onboarding, doctors, shims, rebuild sequence |
| **[../README.md](../README.md)** | Everyone | Script index |
| **[AUDIT.md](AUDIT.md)** | Maintainers | Security matrix, open items, menu QA |

**Lanes:** [system/README.md](../system/README.md) · [dev/README.md](../dev/README.md) · [android/README.md](../android/README.md)

**MobSF:** [GUIDE.md](../mobsf/GUIDE.md) · [STACK.md](../mobsf/STACK.md) · [TROUBLESHOOTING.md](../mobsf/TROUBLESHOOTING.md)

**Other:** [logs/README.md](../logs/README.md) · [legacy/README.md](../legacy/README.md)

```text
README.md          script index
docs/              GETTING-STARTED + AUDIT
system|dev|android lane quick refs
mobsf/             GUIDE + STACK + TROUBLESHOOTING
logs/              logging engine
```
